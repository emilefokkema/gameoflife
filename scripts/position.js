define(["tree/hashlife","coordinates","selection"],function(hashLife, coordinates, selection){
	...
	var alongIntegerX = {
		initialIndex:function(viewBox){
			return Math.ceil(viewBox.x);
		},
		includeIndex:function(index, viewBox){
			return index >= viewBox.x && index <= viewBox.x + viewBox.width;
		},
		transform:function(index, context){
			context.translate(index, 0);
		}
	};
	var alongIntegerY = {
		initialIndex:function(viewBox){
			return Math.ceil(viewBox.y);
		},
		includeIndex:function(index, viewBox){
			return index >= viewBox.y && index <= viewBox.y + viewBox.height;
		},
		transform:function(index, context){
			context.translate(0, index);
		}
	};
	var drawLines = function(context){
		...
		var i,j;
		for(i of context.transformMultiple(alongIntegerX)){
			context.beginPath();
			context.rect(0,-Infinity,Infinity,Infinity);
			context.stroke();
		}
		for(j of context.transformMultiple(alongIntegerY)){
			context.beginPath();
